...
def get_data_and_attributes_from_source(source_file: h5py.File, element:TranslationElement):
    ...
    if isinstance(dataLocation, h5py.Dataset):
        ...
        logging.info(f"Path {element.source} does not exist in the input file, using default value {element.default_value} instead")
        data = element.default_value

    if data is None:
        logging.warning(f"Path {element.source} does not exist in the input file, and no default value was specified. skipping...")
...
def add_dimensions_if_needed(data, element:TranslationElement):
    ...
    if not np.isfinite(element.minimum_dimensionality):
        # no change needed
        return data
    ...
    if isinstance(data, np.ndarray):
        dtype = data.dtype
    try:
        logging.debug(f"trying to add {element.minimum_dimensionality} dimensions to {data=} of dtype {dtype=}")
        data = np.array(
            data, dtype=dtype, ndmin=int(element.minimum_dimensionality)
        )
    except:  # didn't work...
...
def select_source_units(element:TranslationElement, attributes:Union[dict|None] =None):
    """
...
def copy_dataset(source_dataset: h5py.Dataset, dest_file: h5py.File, dest_path: str,
    ...
    # Copy dataset
    write_dataset(dest_file, dest_path, data, compression=source_dataset.compression, attributes=attributes)
...
def copy_hdf5_tree(input_file:Path, output_file:Path, input_path:str, output_path:str):
    ...
    with h5py.File(input_file, 'r') as h5_in, h5py.File(output_file, 'a') as h5_out:
        if not isinstance(h5_in.get(input_path, default='NonExistentGroup'), h5py.Group):
